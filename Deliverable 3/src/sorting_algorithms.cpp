...
/*
    Purpose: This merge function joins two sorted halves of the array back together.
    Working: It compares elements from the two temporary subarrays (leftArr and rightArr)
             and places them back into the original array (arr) in sorted order.
    DSA Concept: Core step of Merge Sort, relies on temporary arrays and comparison.
*/
void SortingAlgorithms::merge(UserAction arr[], size_t left, size_t mid, size_t right) {
    ...
    // Create temporary arrays (raw arrays instead of vectors)
    // Satisfies requirement for explicit raw array usage
    UserAction* leftArr = new UserAction[leftSize];
    UserAction* rightArr = new UserAction[rightSize];
    ...
    // Merge both halves into the original array, sorting by timestamp
    while (i < leftSize && j < rightSize) {
...
/*
    Purpose: Sorts the array of UserAction items by timestamp (ascending).
    Working: Splits the array recursively into two halves until single elements are reached,
             then uses 'merge' to combine them in sorted order.
    DSA Concept: Implementation of the Merge Sort algorithm (O(n log n)) and **Recursion**.
*/
void SortingAlgorithms::mergeSort(UserAction arr[], size_t left, size_t right) {
...
/*
    Purpose: Swaps two UserAction items.
    DSA Concept: Fundamental **Pointer/Array** utility function.
*/
void swap(UserAction& a, UserAction& b) {
    UserAction temp = a;
    a = b;
...
/*
    ...
    Working: Selects the rightmost element as the pivot, rearranges the array so all
             elements smaller than the pivot come before it, and all greater elements
             come after it.
    DSA Concept: Core Partition step for Quick Sort.
*/
size_t SortingAlgorithms::partition(UserAction arr[], size_t low, size_t high) {
    // Choose the rightmost element as the pivot (using timestamp for comparison)
    double pivot = arr[high].timestamp;
